	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btn2.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// 버튼 제어를 handler에서 수행해준다. 1번 메세지를 보내면 사용자가 
				//stop버튼을 누른 것과 같은 것이기
				// 때문에 handler 에서 else if(msg.what == 1)로 받아서 버튼 핸들링을 해준다.
			}
		});
		// start
		btn3.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				// INVISIBLE : 버튼의 자리는 유지한채 안보여지는 상태
				// GONE : 버튼의 자리가 없어지면서 안보여지는 상태
